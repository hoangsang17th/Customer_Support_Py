...
class Video():
    def __init__(self, capture):
        ...
        if self.checkTrainerFound == True:
            self.recognizer.read('dist/Main/trainer/trainer.yml')
    ...
    def captureFace(self, face_id):
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                count += 1
                cv2.imwrite("dist/Main/dataset/User." + str(face_id) +
                            '.' + str(count) + ".jpg", gray[y:y + h, x:x + w])

            # Chụp 30 ảnh mẫu và dừng video chụp ảnh
            if count >= 30:
                cv2.imwrite("image/User." + str(face_id) +
    ...
    def trainingFace(self):
        ...
        recognizer.write('dist/Main/trainer/trainer.yml')
